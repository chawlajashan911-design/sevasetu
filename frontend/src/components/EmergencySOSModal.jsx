...
export const EmergencySOSModal = ({
  ...
  const lat = gpsLocation?.lat || 18.5204;
  const lng = gpsLocation?.lng || 73.8567;
  const accuracy = gpsLocation?.accuracy || 15;
  const villageName = gpsLocation?.villageName || 'Maharashtra';
  const isOutsideMH = gpsLocation?.isOutsideMH;
  const nearestBorderDistanceKm = gpsLocation?.nearestBorderDistanceKm;

  const mapsUrl = `https://www.google.com/maps?q=${lat},${lng}`;
  const smsBody = encodeURIComponent(
    `EMERGENCY MEDICAL SOS: Immediate ambulance required at Village: ${villageName}. Coordinates: ${lat.toFixed(5)}, ${lng.toFixed(5)}. Maps: ${mapsUrl}`
  );
