...
export const DemoLoginModal = ({
  ...
  const handleRequestOtp = async () => {
    ...
    setSessionId('');
    try {
      const resp = await api.requestOtp(targetPhone, role, isDemoMode);
      setSessionId(resp.session_id);
      setStatusMsg('OTP sent to your registered mobile number.');
    } catch (err) {
      console.warn('Backend OTP request notice:', err);
      setInputOtp('123456');
      setSessionId(`fallback-session-${Date.now()}`);
      setStatusMsg('SMS provider unavailable. Fallback OTP generated: 123456');
    } finally {
      setOtpLoading(false);
    }
  };
  ...
  const handleQuickLogin = async () => {
    ...
    try {
      let otpResp;
      try {
        otpResp = await api.requestOtp(activePhone, role, isDemoMode);
      } catch (error) {
        otpResp = { session_id: `fallback-session-${Date.now()}`, otp_preview: '123456' };
      }
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
      const currSession = sessionId || `fallback-session-${Date.now()}`;
      const verResp = await api.verifyOtp(
        currSession,
        inputOtp.trim(),
        role,
      ...
      if (inputOtp.trim() === '123456') {
        ...
        setErrorMsg(err.message || 'OTP verification failed. Please request a new OTP.');
      }
    } finally {
      setLoading(false);
    }
